...
import { executeTasks } from '@/lib/tools/tasks'
import { executeFinance } from '@/lib/tools/finance'
import { executeHabits } from '@/lib/tools/habits'
import { executeGoals } from '@/lib/tools/goals'
...
async function summarizeDashboard(supabase: ReturnType<typeof createServerClient>, userId: string): Promise<string> {
  ...
  const lines = [
    ...
    `**This Month Finance:** Income RM${income.toFixed(2)} · Expenses RM${expenses.toFixed(2)} · Net RM${(income - expenses).toFixed(2)}`,
...
async function generateInsights(supabase: ReturnType<typeof createServerClient>, userId: string): Promise<string> {
  ...
  if (topCategory) {
    insights.push(`💸 Top spending category this month: ${topCategory[0]} (RM${(topCategory[1] as number).toFixed(2)})`)
  }
